...
export function WorkspaceTabs() {
  ...
  return (
    <div className="absolute top-2 left-1/2 -translate-x-1/2 z-[1000] flex items-center gap-1.5 px-2 py-1 bg-background/80 backdrop-blur-md rounded-full border-2 border-border shadow-[var(--brutal-shadow)] pointer-events-auto">
      <AnimatePresence mode="popLayout" initial={false}>
        {openWorkspaces.map((ws) => (
          <motion.div
             ...
             className={cn(
               "group relative flex items-center gap-2 px-3 py-1.5 rounded-full border-2 transition-all cursor-pointer select-none",
               ws.id === workspaceId 
                 ? "bg-card border-primary text-foreground shadow-[2px_2px_0px_hsl(var(--primary))]" 
